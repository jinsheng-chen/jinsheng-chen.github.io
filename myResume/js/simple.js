...
window.onload = function(){
    ...
    oTitle.innerHTML = Json.myInfo.name + '的个人简历';
    ...
    for(var i=0; i<Json.work.length; i++){
        ...
        var item = document.createElement('div');
        item.className = 'item';
        item.innerHTML = '<div class="content"><div class="head"><img src="'+ Json.work[i].logo +'" alt=""/><div class="sta"></div><div class="name"><h4>'+ Json.work[i].company +'</h4><p>'+ Json.work[i].date +'</p></div></div><dl class="text"><dt>工作描述：</dt><dd>'+ Json.work[i].detail +'</dd></dl></div><i></i>';
        workBox.appendChild(item);

    }


    if(body.offsetHeight<1900){
