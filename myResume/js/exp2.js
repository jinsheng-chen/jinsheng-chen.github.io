...
function ground(){
    ...
    if(aChild.length>1){
        ...
        var initial = aChild[1];

    }else{

        var initial = aChild[0];
    ...
    if(aChild.length>1){
        ...
        oUl.style.left = '50%';
        oUl.style.marginLeft = -iWidth/2 + 'px';
        aChild[0].style.webkitTransform = 'translateY(' + 0 + 'px)';
        aChild[0].style.transform = 'translateY(' + 0 + 'px)';
        aChild[0].posTop = 0;
